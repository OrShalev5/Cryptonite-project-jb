$(document).ready(() => {
  ...
  $(".nav-link").on("click", (event) => {
    ...
    if (menuId == "livereports") {
      // AJAX call to fetch live reports
      $.ajax({
        type: "GET",
              ...
              $.ajax({
                ...
                success: function (result) {
                  if (result.Response == "Error") {
                    ...
                    $(".loader").hide();
                    $("#maincontainer").html(
                      `<div class="noneselectedmsg"> <h2>No data on selected currencies - please try other coins!</h2> </div>`
                    );
                  } else {
                    $("#maincontainer").html(
  ...
  function startDefault() {
    // AJAX call to fetch default content
    $(".loader").show();
    clearInterval(IntervalId);
    $.ajax({
  ...
  startDefault();
   ...
   // Function to create HTML containers displaying coin data
      ...
      `<div class="col-sm-4"  id="${card.symbol.toUpperCase()}" >
            <div class="card">
                <div class="card-body">
                    ...
                    <button class="btn" id="moreinfobutton${
                      card.id
  ...
  // Function to fetch more info about a coin
    ...
    $(`#moreinfobutton${idCoin}`).on("click", function () {
      ...
      if (backUpCoin != null && timeNow - backUpCoin.time < 120000) {
        ...
        $(`#${idCoin}.card`).html(`
              ...
              <div>$ ${backUpCoin.market_data.current_price.usd.toFixed(
                4
              )}</div>
              <div>€ ${backUpCoin.market_data.current_price.eur.toFixed(
                4
              )}</div>
              <div>₪ ${backUpCoin.market_data.current_price.ils.toFixed(
                4
              )}</div>
              `);
      } else {
...
// Function to update the displayed selected coins
...
// Function to ensure toggles reflect the selected coins
